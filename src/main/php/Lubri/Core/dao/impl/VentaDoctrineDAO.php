<?php
namespace Lubri\Core\dao\impl;

use Lubri\Core\utils\LubriUtils;

use Doctrine\ORM\Query\Expr\Andx;

use Lubri\Core\dao\IVentaDAO;
...
class VentaDoctrineDAO extends CrudDAO implements IVentaDAO{
	...
	protected function enhanceQueryBuild(QueryBuilder $queryBuilder, ICriteria $criteria){
        ...
        $marcaOPatente = $criteria->getMarcaOPatente();
        if( !empty($marcaOPatente) ){
            $queryBuilder->orWhere("upper(v.marca)  like :nombre");
            $queryBuilder->setParameter( "nombre" , "%$marcaOPatente%" );
            $queryBuilder->orWhere("upper(v.patente)  like :nombre1");
            $queryBuilder->setParameter( "nombre1" , "%$marcaOPatente%" );
