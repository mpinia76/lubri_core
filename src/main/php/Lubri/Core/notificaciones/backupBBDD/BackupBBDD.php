<?php
namespace Lubri\Core\notificaciones\backupBBDD;

use Lubri\Core\utils\XTemplate;
...
class BackupBBDD{
	...
	function backupDatabaseTables($dbHost,$dbUsername,$dbPassword,$dbName, $filename,$tables = '*' ){
	    ...
	    //save file
		$nameFile ='db_Lubri_' . date("Y-m-d-H-i-s") .'.sql';
	...
	public function send(){
		...
		$this->setPathBackup(LubriConfig::getInstance()->getAppPathCore()."/backup/");

		$filename = 'db_lubri_' . date("Y-m-d-H-i-s") . '.zip';
		$backupFile = $this->getPathBackup().$filename;

		self::backupDatabaseTables(LubriConfig::getInstance()->getDbHost(),LubriConfig::getInstance()->getDbUser(),LubriConfig::getInstance()->getDbPassword(),LubriConfig::getInstance()->getDbName(),$filename);
		...
		$template->assign("titulo", "En el archivo adjunto se encuentra el BackUp de la BBDD cose_lubri realizado el ".date('d/m/Y H:i:s'));
		...
		LubriUtils::sendMail(LubriConfig::TEST_MAIL_TO,LubriConfig::TEST_MAIL_TO, $subjectMail, $mensaje, $attachs);
		unlink($backupFile);
